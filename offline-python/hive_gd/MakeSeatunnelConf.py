...
#全量表集合
SYNC_TYPE_MAP = {
    # "product_category": "full",
    "product_info": "full",
    "product_price_power": "full",
    "product_visitor_detail": "inc",
    "product_transaction_detail": "inc",
    "product_search_term": "inc",
    "product_warning": "inc",
    "product_inventory": "inc"
}
...
def generate_seatunnel_config(table_name,mysql_user,columns):
    ...
    # 根据表类型生成不同的查询语句
    if table_type=="full":
        # 全量表使用create_time转换成分区字段DATE_FORMAT(NOW(), '%Y%m%d')DATE_FORMAT(NOW(), '%Y%m%d')
        query = f"SELECT {columns_list},'20250807'  as ds FROM `{database}`.`{table_name}` "
        table_type_note = "全量表 (使用当前日期作为分区)"
    else:
        # 增量表使用当前日期作为分区字段{formatted_time}
        query = f"SELECT {columns_list}, DATE_FORMAT(create_time, '%Y%m%d') as ds FROM `{database}`.`{table_name}` "
        query+=f"where DATE_FORMAT(create_time, '%Y%m%d')='20250807' "
        table_type_note = "增量表 (使用create_time作为分区源)"#DATE_FORMAT(now(), '%Y%m%d')
